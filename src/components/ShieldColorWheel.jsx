...
import { getShieldHueSectors, SHIELD_NEUTRALS } from '../data/shieldHueWheel';
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          <span className="text-[11px] font-semibold text-slate-800 truncate">
            S{color.shieldNo} {color.koName || color.name}
          </span>
...
function ShieldColorWheel({ selectedKeys, onToggleMixColor }) {
  const sectors = useMemo(() => getShieldHueSectors(), []);
  const size = 460;
  const cx = size / 2;
  const cy = size / 2;
  ...
  const selectedList = useMemo(() => {
    const all = [...sectors.flatMap((s) => s.colors), ...SHIELD_NEUTRALS];
      ...
      <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-center gap-5">
        <div className="w-full max-w-[460px] aspect-square shrink-0">
          <svg
            ...
            role="img"
            aria-label="쉴드 에픽 아크릴 36색 색상환. 12시 Yellow부터 시계방향."
          >
                  ...
                  {sector.colors.map((color, ring) => {
                    ...
                    return (
                      <g key={key}>
                        ...
                        >
                          <title>
                            {`S${color.shieldNo} · ${color.koName} ${color.name} · ${sector.cat} · ${color.tone}${
            ...
            {SHIELD_NEUTRALS.map((color, i) => {
              const slice = 360 / SHIELD_NEUTRALS.length;
              const startAngle = i * slice - 90;
              const endAngle = startAngle + slice;
              const key = `n-${color.shieldNo}`;
              const isActive = selectedKeys?.has(mixSwatchKey(color));
              ...
              return (
                <g key={key}>
                  <path
                    ...
                    onClick={() => handleSelect(color)}
                  >
                    <title>{`S${color.shieldNo} · ${color.koName} · 무채/어스 · ${color.note}`}</title>
                  </path>
                  <text
                    x={tp.x}
                    y={tp.y}
        ...
        <div className="w-full max-w-sm space-y-3 text-[11px] text-slate-600">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mb-1.5">
              화이트·펄·그레이·블랙·메탈 2색과 어스 3색을 중앙에 두었습니다.
            </p>
            <div className="space-y-0.5">
              {SHIELD_NEUTRALS.map((color) => (
                <ColorRow
                  key={color.shieldNo}
                  color={color}
...
export default ShieldColorWheel;
