...
import { getMijelloHueSectors, MIJELLO_NEUTRALS } from '../data/mijelloHueWheel';
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          <span className="text-[11px] font-semibold text-slate-800 truncate">
            M{color.mijelloNo} {color.koName || color.name}
          </span>
...
function MijelloColorWheel({ selectedKeys, onToggleMixColor }) {
  const sectors = useMemo(() => getMijelloHueSectors(), []);
  const size = 460;
  const cx = size / 2;
  const cy = size / 2;
  ...
  const selectedList = useMemo(() => {
    const all = [...sectors.flatMap((s) => s.colors), ...MIJELLO_NEUTRALS];
      ...
      <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-center gap-5">
        <div className="w-full max-w-[460px] aspect-square shrink-0">
          <svg
            ...
            role="img"
            aria-label="미젤로 미션골드 34색 색상환. 12시 Yellow부터 시계방향."
          >
                  ...
                  {sector.colors.map((color, ring) => {
                    ...
                    return (
                      <g key={key}>
                        ...
                        >
                          <title>
                            {`M${color.mijelloNo} · ${color.koName} ${color.name} · ${sector.cat} · ${color.tone}${
            ...
            {MIJELLO_NEUTRALS.map((color, i) => {
              const slice = 360 / MIJELLO_NEUTRALS.length;
              const startAngle = i * slice - 90;
              const endAngle = startAngle + slice;
              const key = `n-${color.mijelloNo}`;
              const isActive = selectedKeys?.has(mixSwatchKey(color));
              ...
              return (
                <g key={key}>
                  <path
                    ...
                    onClick={() => handleSelect(color)}
                  >
                    <title>{`M${color.mijelloNo} · ${color.koName} · 어스/무채 · ${color.note}`}</title>
                  </path>
                  <text
                    x={tp.x}
                    y={tp.y}
        ...
        <div className="w-full max-w-sm space-y-3 text-[11px] text-slate-600">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mb-1.5">
              이 34색에는 흰색·검정·회색이 없어, 채도가 낮은 어스 4색을 중앙에 두었습니다.
            </p>
            <div className="space-y-0.5">
              {MIJELLO_NEUTRALS.map((color) => (
                <ColorRow
                  key={color.mijelloNo}
                  color={color}
...
export default MijelloColorWheel;
