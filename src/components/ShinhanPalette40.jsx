...
import { PALETTE_ROWS, shortPaletteName, toneMark } from '../data/shinhanPaletteMwp1640';
...
function Well({ color, active, onSelect }) {
  ...
  const no = color.shinhanNo != null ? String(color.shinhanNo) : '·';
  ...
  return (
    <button
      ...
      disabled={empty}
      title={`${color.shinhanNo ? `H${color.shinhanNo} · ` : ''}${color.koName || ''} ${color.name || ''} · ${color.hue}${mark ? ` · ${mark}` : ''}`}
...
function ShinhanPalette40({ selectedKeys, onToggleMixColor }) {
  return (
    <div className="rounded-3xl bg-white border border-slate-200 p-4 md:p-5 shadow-md w-full">
      ...
      <div className="space-y-3">
        <PaletteRow rowKey="r1" row={PALETTE_ROWS.row1} selectedKeys={selectedKeys} onSelect={onToggleMixColor} />
        <PaletteRow rowKey="r2" row={PALETTE_ROWS.row2} selectedKeys={selectedKeys} onSelect={onToggleMixColor} />
        <PaletteRow rowKey="r3" row={PALETTE_ROWS.row3} selectedKeys={selectedKeys} onSelect={onToggleMixColor} />
      </div>

      <p className="mt-3 text-[10px] text-slate-500 leading-relaxed">
        고른 물감으로 위 Mixing Preview · 디지털 팔레트가 목표색을 조색합니다. 안료 최대 4색, 물은 별도.
        노란 테두리는 32색에 없는 예비 무채입니다.
      </p>
    </div>
  );
}

export default ShinhanPalette40;
